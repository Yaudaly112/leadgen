    ...
    async def send_cold_email(self, lead_id: int) -> dict:
        ...
        async with async_session() as session:
            ...
            result = await email_sender.send_email(
                ...
                subject=email_content["subject"],
                html_body=html_body,
                custom_args={"lead_id": str(lead_id), "outreach_log_id": str(log.id)},
            )
        ...
        try:
            ...
            if not run.skip_discovery:
                ...
                try:
                    ...
                    print(f"[PIPELINE ERROR] Discovery: {type(e).__name__}: {e}")
                    traceback.print_exc()
            ...
            if not run.skip_enrichment:
                ...
                for i, lead in enumerate(leads_to_enrich):
                    ...
                    if (i + 1) % 5 == 0 or i == len(leads_to_enrich) - 1:
                        await self._update_run(run_id, enrichment_done=enriched, enrichment_errors=enrich_errors)
            ...
            if not run.skip_demo:
                ...
                for i, lead in enumerate(leads_to_demo):
                    ...
                    if (i + 1) % 5 == 0 or i == len(leads_to_demo) - 1:
                        await self._update_run(run_id, demo_done=demos, demo_errors=demo_errors)
            ...
            if not run.skip_email:
                ...
                for i, lead in enumerate(leads_to_email):
                    ...
                    if (i + 1) % 5 == 0 or i == len(leads_to_email) - 1:
                        await self._update_run(run_id, email_sent=sent, email_queued=queued, email_suppressed=suppressed, email_errors=email_errors)
            ...
            await self._update_run(run_id, status=run_status, completed_at=datetime.utcnow(), error_log=json.dumps(errors) if errors else None)
            ...
            return {"run_id": run_id, "status": run_status, "errors": errors}
